...
"""# Convert input of board Upper and Blockers for printing
def convertInput(boardConfig):
    board = {}
    for token in boardConfig[UPPER]:
        token[TOKEN] = token[TOKEN].upper()
        board[(token[R]), token[Q]] = '(' + token[TOKEN] + ')'
    for token in boardConfig[BLOCK]:
        token[TOKEN] = "|"
        board[(token[R], token[Q])] = '(' + token[TOKEN] + ')'
    return board"""
